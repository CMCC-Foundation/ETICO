...
if __name__ == "__main__":
    ...
    print("IS ODS EQUAL TO DS? %s" % (ds.equals(ods)))
    ...
    while True:
        ...
        # debug print
        print("============================================================")
        print("[__main__] === Iteration %s with LAT_IDX: %s and LON_IDX: %s with DEPTH %s" % (iterat, lat_idx, lon_idx, ds.bathy.isel(lat=lat_idx, lon=lon_idx).values))
        print("IS ODS EQUAL TO DS? %s" % (ds.equals(ods)))
        ...
        print("[__main__] === Will shift lat/lon by: %s,%s" % (shifted_lat, shifted_lon))
        print("[__main__] === New matrix will be centered on %s,%s with depth %s" % (shifted_lat, shifted_lon, get_depth(ds, (shifted_lat, shifted_lon))))
        
        # Update the current point to the one with minimum bathymetry
        lat_idx, lon_idx = shifted_lat, shifted_lon
        depth = get_depth(ds, (lat_idx, lon_idx))
        print("New matrix will be centered on %s,%s with depth %s" % (lat_idx, lon_idx, depth))
